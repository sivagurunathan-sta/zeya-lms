...
const CourseView = () => {
  ...
  const fetchCourseData = async () => {
    try {
      ...
      const courseRes = await axios.get(`http://localhost:5000/api/intern/courses/${courseId}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      const tasksRes = await axios.get(`http://localhost:5000/api/intern/courses/${courseId}/tasks`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      ...
      const allCompleted = tasksRes.data.tasks.every(t => t.isCompleted);
      ...
      <div className="course-header">
        ...
        {allTasksCompleted && (
          <div className="certificate-cta">
            <button onClick={() => setPaymentModal(true)} className="btn-get-certificate">
              🏆 Get Certificate - ₹{course.price}
            </button>
          </div>
        )}
      </div>
